...
Видим что в шаблоне указана старая версия образа 'nginx'. Через изменения файла Chart.yaml изменим версию 'nginx' c 1.16.0 на 1.19.1:

```shell
zag1988@k8s-test:~/main/2.5/charts/01-simple$ cat Chart.yaml 
apiVersion: v2
name: hard
...
В итоге, создали шаблон, который  создает объекты 'pod' и 'service' 
